...
def short_set_entries_exits_array(self, candles: np.array, ind_set_index: int):
    try:
        ...
        # Indicator settings for this index
        self.ema_length = self.indicator_settings_arrays.ema_length[ind_set_index]
        self.fast_length = self.indicator_settings_arrays.fast_length[ind_set_index]
        self.macd_below = self.indicator_settings_arrays.macd_below[ind_set_index]
        self.signal_smoothing = self.indicator_settings_arrays.signal_smoothing[ind_set_index]
        ...
        self.ema = ema_tv(source=closing_prices, length=self.ema_length)

        # Determine previous MACD and Signal for comparison
        prev_macd = np.roll(self.macd, 1)
        prev_signal = np.roll(self.signal, 1)
        prev_macd[0] = np.nan
        prev_signal[0] = np.nan
        ...
        current_macd_below_signal = self.macd < self.signal
        high_price_below_ema = high_prices < self.ema
        macd_below_number = self.macd < self.macd_below

        # Set entries based on conditions
        self.entries = (
            (high_price_below_ema == True) &
            (prev_macd_above_signal == True) &
            (current_macd_below_signal == True) &
            (macd_below_number == True)
        )
...
def short_entry_message(self, bar_index: int):
    logger.info("\n\nShort Entry Signal!!!")
    logger.info(f"Bar Index: {bar_index}")
